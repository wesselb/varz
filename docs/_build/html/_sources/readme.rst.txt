...
Painless optimisation of constrained variables in AutoGrad, TensorFlow,
and PyTorch

*Note:* Varz requires Python 3.6 or higher and TensorFlow 2 if
TensorFlow is used.
...
-  `Manual <#manual>`__
   ...
   -  `Automatic Naming of Variables <#automatic-naming-of-variables>`__
   -  `Optimisers <#optimisers>`__
   -  `PyTorch Specifics <#pytorch-specifics>`__
   -  `Getting and Setting Variables as a
      Vector <#getting-and-setting-variables-as-a-vector>`__
   -  `Getting Variables from a Source <#get-variables-from-a-source>`__
...
use with AutoGrad, use a ``np.*`` data type; for use with PyTorch, use a
``torch.*`` data type; and for use with TensorFlow, use a ``tf.*`` data
type. In this example we'll use AutoGrad.
...
``x``. This asserts that a variable with a name ``x`` already exists,
and will throw a ``KeyError`` otherwise.
...
The names of all variables can be obtained with ``Vars.names``, and
variables can be printed with ``Vars.print``.
...
-  **Positive variables:** A variable that is constrained to be
   *positive* can be created using ``Vars.positive`` or ``Vars.pos``.
...
-  **Bounded variables:** A variable that is constrained to be *bounded*
   can be created using ``Vars.bounded`` or ``Vars.bnd``.
...
The names for ``x`` and ``y`` are necessary, because otherwise new
variables will be created and initialised every time ``objective`` is
ran. Varz offers two ways to not having to specify a name for every
variable: sequential and parametrised specification.

Sequential Specification
^^^^^^^^^^^^^^^^^^^^^^^^

Sequential specification can be used if, upon execution of
``objective``, variables are always obtained in the *same order*. This
...
decorate the function with ``sequential``.
...
``x = vs.get(5)`` and ``y = vs.get()``. This is the problem that
parametrised specification addresses, which allows you to specify
variables as *arguments to your function*. To indicate that an argument
...
The L-BFGS-B algorithm is recommended for deterministic objectives, and
Adam is recommended for stochastic objectives.
...
The follow examples show how a function can be minimised using the
L-BFGS-B algorithm.
